...
import { useReducedMotion, useScroll, useTransform, type Variants } from "framer-motion";

import { SectionTitle } from "@/components/section-title";
import { ProjectCard } from "@/components/card-projeto";
import { CallToAction } from "@/components/call-to-action";
import { useMediaQuery } from "@/hooks/use-media-query";
import identidadeCafeteriaAurora from "@/public/assets/portfolio/mockups/identidade-cafeteria-aurora.webp";
import landingColetivoCriativo from "@/public/assets/portfolio/mockups/landing-coletivo-criativo.webp";
import embalagemSkincareEssenciaPura from "@/public/assets/portfolio/mockups/embalagem-skincare-essencia-pura.webp";
import direcaoArteFestivalAurora from "@/public/assets/portfolio/mockups/direcao-arte-festival-aurora.webp";
import mockupEducaflow from "@/public/assets/portfolio/mockups/mockup-educaflow.webp";
import appMiSaladasMockup from "@/public/assets/portfolio/mockups/app-mi-saladas.webp";
import { motion } from "@/lib/motion";
import { useIdleReady } from "@/app/(site)/hooks/use-idle-ready";

const projects: Array<{
  title: string;
  description: string;
  tags: string[];
  imageSrc: typeof identidadeCafeteriaAurora;
...
}> = [
  {
    ...
    imageSrc: identidadeCafeteriaAurora,
    priority: true,
  },
  {
    ...
    imageSrc: landingColetivoCriativo,
    priority: true,
  },
  {
    ...
    imageSrc: embalagemSkincareEssenciaPura,
  },
  {
    ...
    imageSrc: direcaoArteFestivalAurora,
  },
  {
...
export default function PortfolioPageContent() {
  ...
  const heroVariants = React.useMemo(
    () => (shouldReduceMotion ? { hidden: { opacity: 1, y: 0 }, visible: { opacity: 1, y: 0 } } : fadeIn),
    [shouldReduceMotion]
  );
  ...
  return (
    <div className="mx-auto w-full max-w-7xl space-y-24 px-6 py-24 text-neutral-800 md:space-y-32 md:px-12 md:py-32 dark:text-neutral-300">
      <motion.section
      ...
      >
        <p className="text-xs uppercase tracking-[0.5em] text-neutral-500 dark:text-neutral-500">Seleção de projetos</p>
        ...
        <p className="text-base leading-relaxed text-neutral-600 dark:text-neutral-400 md:text-lg">
          Cada projeto nasce do encontro entre estratégia, estética e propósito. Atuamos em branding, produção física e
          presença digital com o mesmo cuidado - criando sistemas vivos de comunicação, não peças isoladas. Abaixo, uma
          seleção curada de histórias visuais que seguem presentes em diferentes superfícies.
        </p>
      </motion.section>
      ...
      <section id="cases" className="space-y-6 md:space-y-8">
        <div className="max-w-3xl space-y-4">
          ...
          <p className="max-w-3xl text-sm leading-relaxed text-neutral-600 dark:text-neutral-400 md:text-base">
            Selecionamos projetos recentes que demonstram como traduzimos objetivos de negócio em experiências
            memoráveis. Do posicionamento à implementação, cada entrega preserva o DNA da marca.
          </p>
        </div>
        <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-3">
          {projects.map((project, index) => (
            <ProjectCard
              ...
              {...project}
              delay={index * 0.08}
              imagePriority={Boolean(project.priority)}
            />
          ))}
        </div>
      </section>
      ...
      >
        <motion.div
          style={{ y: idleReady ? ctaParallax : 0 }}
